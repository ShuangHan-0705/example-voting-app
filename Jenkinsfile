...
pipeline {
  ...
  stages {
    ...
    stage('Build Result Image') {
      steps {
        sh 'printenv'
        sh 'docker build -t hannah0705/result ./result'
        echo 'Build result completed'
      }
    } 
    stage('Build Vote Image') {
      steps {
        sh 'docker build -t hannah0705/vote ./vote'
      }
    }
    stage('Build Worker Image') {
      steps {
        sh 'docker build -t hannah0705/worker ./worker'
      }
    }
    stage('E2E Testing') {
      steps {
    ...
    stage('Push Result Image') {
      ...
      steps {
        withDockerRegistry(credentialsId: 'dockerhubid', url:'') {
          sh 'docker push hannah0705/result'
        }
      }
    }
    stage('Push Vote Image') {
      ...
      steps {
        withDockerRegistry(credentialsId: 'dockerhubid', url:'') {
          sh 'docker push hannah0705/vote'
        }
      }
    }
    stage('Push Worker Image') {
      ...
      steps {
        withDockerRegistry(credentialsId: 'dockerhubid', url:'') {
          sh 'docker push hannah0705/worker'
        }
      }
    }
    stage('Deployment') {
